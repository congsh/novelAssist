  ...
  db.run('PRAGMA foreign_keys = ON', (err) => {
    ...
    db.run('BEGIN TRANSACTION', (err) => {
      ...
      db.run(`DELETE FROM relationships WHERE 
        (entity1_type = 'outline') OR (entity2_type = 'outline')`, (err) => {
        if (err) {
          console.error('删除大纲关联关系失败:', err.message);
          rollbackAndExit(1);
        }
        
        console.log('已删除大纲关联关系');
        
        // 2. 删除所有大纲数据
        db.run('DELETE FROM outlines', (err) => {
          if (err) {
            console.error('删除大纲数据失败:', err.message);
            rollbackAndExit(1);
          }
          
          console.log('已删除所有大纲数据');
